...
def make_output_rootfile(h_PASS, h_FAIL, sample, out_dir):
    ...
    elif sample.find("data") != -1:
        signal_name = sample.split("data_")[-1]
        filename = f"BH_Data_train_{signal_name}.root"
...
if __name__ == '__main__':
    ...
    lumi = 137.62 * 1000 # 59.8 * 1000 # unit pb^-1
    outDir = "./histograms_for_2DAlphabet_v14"
    blind = False
    ...
    if sample == "QCD":
        ...
        signal_dir = "/home/users/dazhang/works/phaseSpace/BlackHoleSearch/PhaseSpaceOT/EventClassification/eval/condor_singleMassPoints/"
        sample_types = ["B1_MD2000_MBH3000_n2", "B1_MD2000_MBH4000_n2", "B1_MD2000_MBH5000_n2", "B1_MD2000_MBH6000_n2", \
                        "B1_MD2000_MBH7000_n2", "B1_MD2000_MBH8000_n2", "B1_MD2000_MBH9000_n2", "B1_MD2000_MBH10000_n2", "B1_MD2000_MBH11000_n2"]
        
        for sample_type in sample_types:
            rootfile_dir = os.path.join(signal_dir, "train_"+sample_type, "signal", "BlackHole_"+sample_type)
            ...
            hist_PASS.Scale(crossSectionArray_blackmax["BlackHole_"+sample_type] * lumi / ngen)
            hist_FAIL.Scale(crossSectionArray_blackmax["BlackHole_"+sample_type] * lumi / ngen)
            
            make_output_rootfile(hist_PASS, hist_FAIL, "BlackHole_"+sample_type, outDir)

    elif sample == "data":
        base_dir = "/home/users/dazhang/works/phaseSpace/BlackHoleSearch/PhaseSpaceOT/EventClassification/eval/condor_singleMassPoints/"
        sample_types = ["B1_MD2000_MBH3000_n2", "B1_MD2000_MBH4000_n2", "B1_MD2000_MBH5000_n2", "B1_MD2000_MBH6000_n2", \
                        "B1_MD2000_MBH7000_n2", "B1_MD2000_MBH8000_n2", "B1_MD2000_MBH9000_n2", "B1_MD2000_MBH10000_n2", "B1_MD2000_MBH11000_n2"]
        
        for sample_type in sample_types:
        
            hist_PASS = ROOT.TH2F(f"Hist_PASS_data_{sample_type}", "ST vs Multiplicity 2D Hist", int((xmax-xmin)/50.), xmin, xmax, 20, 0, 20)
            hist_FAIL = ROOT.TH2F(f"Hist_FAIL_data_{sample_type}", "ST vs Multiplicity 2D Hist", int((xmax-xmin)/50.), xmin, xmax, 20, 0, 20)
            
            tchain = ROOT.TChain("Events")
            # List of directories to process
            data18_dir = os.path.join(base_dir, "train_"+sample_type, "data18")
            data16_dir = os.path.join(base_dir, "train_"+sample_type, "data16")
            data16post_dir = os.path.join(base_dir, "train_"+sample_type, "data16post")
            data17_dir = os.path.join(base_dir, "train_"+sample_type, "data17")
            ...
            tchain.Draw(f"Multiplicity:ST >> Hist_PASS_data_{sample_type}",f"{PASS_blind}")
            tchain.Draw(f"Multiplicity:ST >> Hist_FAIL_data_{sample_type}",f"{FAIL}")
            
            make_output_rootfile(hist_PASS, hist_FAIL, "data_"+sample_type, outDir)
