...
if not options.blind:
    ...
    if ("BH" in cstr) :
      g_mclimit.GetXaxis().SetRangeUser(4.5, 11.5)
...
CMS_lumi.CMS_lumi(climits, 18, 11, sim=False)
...
climits.SaveAs("limits_combine_"+options.lumi.replace('.','p')+"fb_"+options.signals[options.signals.find('/')+1:options.signals.find('.')]+'_'+cstr+".C")
