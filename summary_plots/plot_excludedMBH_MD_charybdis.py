...
for nModel in [1, 2, 3, 4, 6]:
    for nExtra in [2, 4, 6]:
        dict_MD_MBHexcluded = {}
        for MD in MD_list:
            subdir = f'rpfExpo_BinningV5_Blind_InV18_Multi4_FullScan_C{nModel}_n{nExtra}_MD{MD:.0f}TeV'
            filename = f'limits_combine_137p6fb_signals_C{nModel}_n{nExtra}_MD{(MD*1000):.0f}_BH.C'
...
for key, dict_MD_MBHexcluded in dict_model.items():
    print("Model: C{}, nExtra: {}".format(key[0], key[1]))
    for MD, MBHexcluded in dict_MD_MBHexcluded.items():
        print(f"MD = {MD} TeV, MBH excluded = {MBHexcluded} TeV")
    print()
...
color = {1: 'b', 2: 'gold', 3: 'm', 4: 'r', 5: 'lime', 6: 'k'}
marker = {1: 's', 2: '^', 3: 'o', 4: 'v', 5: 'P', 6: 'D'}
line = {2: '-.', 4: '--', 6: '-'}
...
caption1 = [
    "Rotating (C1/BH2)",
    "Nonrotating (C2/BH4)",
    "Rotating, evaporation model (C3/BH8)",
    "Rotating, YR model (C4/BH6)",
    # "Rotating, stable remnant (C5/BH10)",
    "Rotating, boiling remnant (C6/BH9)"
]
legend1 = ax.legend([dict_lines[(1,6)], dict_lines[(2,6)], dict_lines[(3,6)], dict_lines[(4,6)], dict_lines[(6,6)]], caption1, loc='lower left', title="Charybdis2", title_fontsize='x-small', fontsize='x-small')
...
ax.set_xlim(1, 9)
ax.set_ylim(6, 11)
plt.savefig("excludedMBH_MD_charybdis.pdf")
