...
if not options.blind:
    ...
    if ("sphaleron" in cstr) :
        g_limit.GetXaxis().SetRangeUser(8.5, 9.5)
        g_limit.SetMaximum(1)
        g_limit.SetMinimum(1e-4)
else:
    print('Blinded')
    g_mclimit.GetXaxis().SetTitle("E_{sph} [TeV]")  # NOT GENERIC
    g_mclimit.GetYaxis().SetTitle("PEF") # NOT GENERIC
    if ("tau" in cstr) :
    ...
    if ("sphaleron" in cstr) :
        g_mclimit.GetXaxis().SetRangeUser(8.5, 9.5)
        g_mclimit.SetMaximum(1)
        g_mclimit.SetMinimum(1e-4)
# Expected
# g_mclimit = TGraph(len(x_mass), x_mass, y_mclimit)
# g_mclimit.SetTitle("")
# g_mclimit.SetMarkerStyle(21)
...
if not options.blind:
    g_limit.GetXaxis().SetTitle("E_{sph} [TeV]")  # NOT GENERIC
    g_limit.GetYaxis().SetTitle("PEF") # NOT GENERIC
...
legend = TLegend(0.5, 0.7, 0.92, 0.89, '')
...
CMS_lumi.CMS_lumi(climits, 2, 11, sim=False)


filename = "limits_combine_"+options.lumi.replace('.','p')+"fb_"+options.signals[options.signals.find('/')+1:options.signals.find('.')]+'_'+cstr
